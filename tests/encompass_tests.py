...
def test_read_oper_expression(expression, idx_list):
    from encompass.pipeline.supporting_functions import read_oper_expression
...
def test_merge(elements_list, merged_list):
    from encompass.pipeline.supporting_functions import merge
...
def test_gcd(numbers_list, gcd):
    from encompass.pipeline.supporting_functions import find_gcd
...
def test_mmCIF_format_parser(pdbi):
    import os
    import subprocess
    from encompass.pipeline.supporting_functions import parse_mmCIF_format
...
def test_header_parsers(pdbi):
    import os
    import subprocess
    from encompass.pipeline.supporting_functions import parse_mmCIF_format, parse_mmCIF_header, parse_PDB_header
    cif_filename = "{0}.cif".format(pdbi.upper())
    if not os.path.exists(cif_filename):
        subprocess.run(["wget", "https://files.rcsb.org/view/{0}".format(cif_filename)])
    all_loops, loop_keys = parse_mmCIF_format(cif_filename)
