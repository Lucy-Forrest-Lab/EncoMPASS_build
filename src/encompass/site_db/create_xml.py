...
def read_exclusions(delete_list): # TODO - consider: replace with function from combine_sources.py moved to supporting function
    exclude_pdbs = set({})
    if delete_list:
...
def squash_webarchive(options, locations, generate=True):
    ...
    #print("singularity", "exec", options['PATHS'][('container', 'singularity_container')], "unsquashfs", "-user-xattrs", "-d", "ENCOMPASS_SQUASHFS/", squash_fn)
        ...
        for i,a in enumerate(dict_value.strip(';').split(';')):
            if check_dir:
                if not os.path.isfile(check_dir + str(a)):
                    if str(a):
                        #eprint(dict_value.strip(';').split(';')) # TODO standardize standard error for these messages
                        print(dict_value.strip(';').split(';'))
                        full_path = check_dir + str(a)
                        #eprint(f"WARNING: No image found for {full_path} in {xml_entry_name}")
...
def cesymm_empty(locations, struct):
    ...
    for suffix in ["_stdout.out", "_output.xml", ".axes", ".fasta"]:
        if os.path.isfile(locations['FSYSPATH']['cesymm'] + struct + suffix):
            txt += '\t\t\t\t\t<Path> {0}{1}{2} </Path>\n'.format(cpath(locations['FSYS']['cesymm'], struct, suffix))
    txt += '\t\t\t\t</DownloadCE-Symm>\n'
    txt += '\t\t\t\t<CE-SymmImage>\n'
    txt += '\t\t\t\t\t<ImageFile />\n'
    txt += '\t\t\t\t\t<PmlFile />\n'
...
def symd_empty(locations, struct):
    ...
    for suffix in ["-info.txt", "-trfm.pdb", "-best.fasta"]:
        if os.path.isfile(locations['FSYSPATH']['symd'] + struct + suffix):
            txt += '\t\t\t\t\t<Path> {0}{1}{2} </Path>\n'.format(cpath(locations['FSYS']['symd'], struct, suffix))
    txt += '\t\t\t\t</DownloadSymD>\n'
    txt += '\t\t\t\t<SymDImage>\n'
    txt += '\t\t\t\t\t<ImageFile />\n'
    txt += '\t\t\t\t\t<PmlFile />\n'
...
def cesymm_xml(locations, struct, symm_dic_pdbi, xml_type, monomeric=False):
    ...
    if 'alignment' in symm_dic_pdbi['cesymm'] and symm_dic_pdbi['cesymm']['alignment']:
        ...
        aln = format_alignments_from_string(symm_dic_pdbi['cesymm']['alignmnet']) # TODO fix when dic key corrected!
    else:
        if xml_type == 'whole':
    ...
    for suffix in ["_stdout.out", "_output.xml", ".axes", ".fasta"]:
        if monomeric:
            path = locations['FSYSPATH']['cesymm'] + struct[0:4] + "/" + struct[0:4] + suffix
        else:
            path = locations['FSYSPATH']['cesymm'] + struct[0:4] + "/" + struct + suffix
        if os.path.isfile(path):
            txt += '\t\t\t\t\t<Path> {0} </Path>\n'.format(cpath(path.replace(locations['FSYSPATH']['cesymm'], locations['FSYS']['cesymm'])))
    txt += '\t\t\t\t</DownloadCE-Symm>\n'
    txt += '\t\t\t\t<CE-SymmImage>\n'
    if monomeric:
        xml_type = "whole"
    ...
    if 'image_files' not in symm_dic_pdbi['cesymm'] or not symm_dic_pdbi['cesymm']['image_files']:
        #symm_dic_pdbi['cesymm']['image_files'], symm_dic_pdbi['cesymm']['pml_files'], symm_dic_pdbi['cesymm']['jmol_files'] = "", "", ""
...
def symd_xml(locations, struct, symm_dic_pdbi, xml_type, monomeric=False):
    ...
    for suffix in ["-info.txt", "-trfm.pdb", "-best.fasta"]:
        if monomeric:
            path = locations['FSYSPATH']['symd'] + struct[0:4] + "/" + struct[0:4] + suffix
        else:
            path = locations['FSYSPATH']['symd'] + struct[0:4] + "/" + struct + suffix
        if os.path.isfile(path):
            txt += '\t\t\t\t\t<Path> {0} </Path>\n'.format(cpath(path.replace(locations['FSYSPATH']['symd'], locations['FSYS']['symd'])))
    txt += '\t\t\t\t</DownloadSymD>\n'
    txt += '\t\t\t\t<SymDImage>\n'
...
def cpath(path):
    abspath = MAINPATH + path
    if not os.path.exists(abspath):
        print(f"[create_xml] WARNING: path {abspath} does not exist")
    elif os.path.getsize(abspath) == 0:
        print(f"[create_xml] WARNING: file {abspath} is empty")
    return path
...
def struct_xml_entry(pdbi, str_data_pdbi, symm_dic_pdbi, locations, neighbors, deletion_info, consensus, monomeric=False):
    ...
    txt += '\t\t\t\t<DownloadPDBFile> {0} </DownloadPDBFile>\n'.format(cpath(locations['FSYS']['whole'] + pdbi + '_enc.pdb'))
    txt += '\t\t\t\t<ImageFile> {0} </ImageFile>\n'.format(locations['FSYS']['images_figs_whole'] + pdbi + '.png')  # Not in use anymore (no cpath)
    txt += '\t\t\t\t<T3dmolFile> {0} </T3dmolFile>\n'.format(cpath(locations['FSYS']['3dmol_whole'] + pdbi + '.json'))
    txt += '\t\t\t</GeneralStructure>\n'
...
def chain_xml_entry(pdbi, ch, str_data_pdbi, symm_dic_pdbi, locations, neighbor_counts, transfer_list=None, monomeric=False):
    ...
    txt += '\t\t\t<GeneralStructure>\n'
    txt += '\t\t\t\t<DownloadPDBFile> {0} </DownloadPDBFile>\n'.format(cpath(locations['FSYS']['whole'] + pdbi + '_enc.pdb'))
    txt += '\t\t\t\t<ImageFile> {0} </ImageFile>\n'.format(locations['FSYS']['images_figs_chain'] + pdbi_ch + '.png') # Not in use anymore (no cpath)
    txt += '\t\t\t\t<T3dmolFile> {0} </T3dmolFile>\n'.format(cpath(locations['FSYS']['3dmol_whole'] + pdbi + '.json'))
    txt += '\t\t\t</GeneralStructure>\n'
    txt += '\t\t\t<PDB_URL> http://www.rcsb.org/pdb/explore/explore.do?structureId={0} </PDB_URL>\n'.format(pdbi.upper())
    txt += '\t\t\t<OPM_URL> http://opm.phar.umich.edu/protein.php?pdbid={0} </OPM_URL>\n'.format(pdbi)
    ...
    txt += '\t\t\t<StructureNeighbors> {0} </StructureNeighbors>\n'.format(strneighs[pdbi_ch])
    txt += '\t\t\t<TotalNeighbors> {0} </TotalNeighbors>\n'.format(totneighs[pdbi_ch])
    txt += '\t\t\t<SequenceNeighborsFile> {0} </SequenceNeighborsFile>\n'.format(cpath(locations['FSYS']['seqneighs'] + 'seqneigh_' + pdbi_ch + '.txt'))
    txt += '\t\t\t<StructureNeighborsFile> {0} </StructureNeighborsFile>\n'.format(cpath(locations['FSYS']['strneighs'] + 'strneigh_' + pdbi_ch + '.txt'))
    txt += '\t\t\t<TotalNeighborsFile> {0} </TotalNeighborsFile>\n'.format(cpath(locations['FSYS']['totneighs'] + 'totneigh_' + pdbi_ch + '.txt'))
    txt += '\t\t\t<Images>\n'
    txt += '\t\t\t\t<RadialDistribution> {0} </RadialDistribution>\n'.format(cpath(locations['FSYS']['polar_figs'] + 'p_' + pdbi_ch + '.png'))
    txt += '\t\t\t\t<RadialDistributionMap> {0} </RadialDistributionMap>\n'.format(cpath(locations['FSYS']['polar_maps'] + pdbi_ch + '_cmap.txt'))
    txt += '\t\t\t\t<AlignmentsInDensityPlot> {0} </AlignmentsInDensityPlot>\n'.format(cpath(locations['FSYS']['densityscatter_figs'] + 'ds_' + pdbi_ch + '.png'))
    txt += '\t\t\t\t<AlignmentsInDensityPlotMap> {0} </AlignmentsInDensityPlotMap>\n'.format(cpath(locations['FSYS']['densityscatter_maps'] + pdbi_ch + '_cmap.txt'))
    txt += '\t\t\t\t<ResDistanceDistribution> {0} </ResDistanceDistribution>\n'.format(cpath(locations['FSYS']['distributions_figs'] + 'distr_' + pdbi_ch + '.png'))

    if os.path.exists(locations['FSYSPATH']['totneighs'] + 'totneigh_' + pdbi_ch + '.txt'):
        seqid_l, tmscore_l = [], []
        with open(locations['FSYSPATH']['totneighs'] + 'totneigh_' + pdbi_ch + '.txt') as f:
            text = f.read().strip()
            for line in text.split("\n"):
                if not line.strip():
                    continue
                _, seqid, tmscore = line.split()
                seqid_l.append(float(seqid))
                tmscore_l.append(float(tmscore))
        if not tmscore_l:
            print(f"INFO: file {locations['FSYSPATH']['totneighs'] + 'totneigh_' + pdbi_ch + '.txt'} is empty")
        elif min(tmscore_l) > 0.999 or max(tmscore_l) < 0.6:
            print(f"INFO: file {locations['FSYS']['distributions_figs'] + 'distr_' + pdbi_ch + '.png'} expected not to be present because all neighbors are identical (see {locations['FSYS']['totneighs'] + 'totneigh_' + pdbi_ch + '.txt'} )")

    txt += '\t\t\t\t<Topology> {0} </Topology>\n'.format(locations['FSYS']['topologies_figs'] + 'top_' + pdbi_ch + '.jpeg') # Not in use anymore (no cpath)
    txt += '\t\t\t</Images>\n'
    txt += '\t\t\t<AdditionalFiles>\n'
    ...
    if ch in str_data[pdbi]['ENCOMPASS']['structure']['redundant_chains']:
        ...
        repr_ch = ch
    txt += '\t\t\t\t<ResDistanceDistribution> {0} </ResDistanceDistribution>\n'.format(cpath(locations['FSYS']['distributions_data'] + 'distr_' + pdbi + '_' + repr_ch + '.txt'))
    txt += '\t\t\t\t<StructureAlignments> {0} </StructureAlignments>\n'.format(cpath(locations['FSYS']['stralns'] + 'stralns_' + pdbi + '_' + repr_ch + '.txt.gz')) ###!
    txt += '\t\t\t\t<Estimators> {0} </Estimators>\n'.format(cpath(locations['FSYS']['estimators'] + 'est_' + pdbi + '_' + repr_ch + '.txt'))
    txt += '\t\t\t</AdditionalFiles>\n'
    txt += '\t\t</StructureInformation>\n'
    ...
    for pdbi in str_data:
        ...
        for ich, tmch in enumerate(str_data[pdbi]['ENCOMPASS']['structure']['ktmchains']):
            ...
            if pdbi_ch not in seqneighs:
                seqneighs[pdbi_ch] = 0
                create_empty_neighborlist_file(locations, pdbi_ch, 'seqneighs', seqneighs_fn_eg, "seq", v)
            if pdbi_ch not in strneighs:
                strneighs[pdbi_ch] = 0
                create_empty_neighborlist_file(locations, pdbi_ch, 'strneighs', strneighs_fn_eg, "str", v)
            if pdbi_ch not in totneighs:
                totneighs[pdbi_ch] = 0
                create_empty_neighborlist_file(locations, pdbi_ch, 'totneighs', totneighs_fn_eg, "tot", v)
...
if __name__ == "__main__":
    ...
    MAINPATH = locations['FSYSPATH']['main'] + "/"
    verbose_level = 1
