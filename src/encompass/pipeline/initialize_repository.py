...
def load_options(correct_types=False, thr_log_status='ERROR'):
    ...
    options['PATHS'][('hpcananas', 'hpc_ananas_path')] = ''
    options['PATHS'][('hpcppm', 'hpc_ppm_path')] = ''
    options['PATHS'][('hpcmuscle', 'hpc_muscle_path')] = ''
    options['PATHS'][('hpccesymm', 'hpc_cesymm_path')] = ''
    options['PATHS'][('hpccesymmsmall', 'hpc_cesymm_small_path')] = ''
    ...
    options['PATHS'][('hpcpymol', 'hpc_pymol_path')] = ''
    options['PATHS'][('hpcdssp', 'hpc_dssp_path')] = ''
    options['PATHS'][('hpcpymol', 'hpc_pymol_path')] = ''
    options['PATHS'][('sigppm', 'sig_ppm_path')] = '/EncoMPASS/ppm/immers'
    options['PATHS'][('sigmuscle', 'sig_muscle_path')] = '/EncoMPASS/muscle5.1/muscle5.1.linux_intel64'
    options['PATHS'][('sigcesymm', 'sig_cesymm_path')] = '/EncoMPASS/cesymm-2.2.3-SNAPSHOT/runCESymm.sh'
    options['PATHS'][('sigcesymmsmall', 'sig_cesymm_small_path')] = '/EncoMPASS/cesymm-2.2.3-SNAPSHOT/runCESymm_small.sh'
    ...
    options['PATHS'][('locuststmp', 'locusts_tmp_path')] = ''
    ...
    options['RUN'][('hpclocal', 'run_on_hpc_local')] = 'False'
    ...
    options['RUN'][('start', 'start_stage')] = '0'
    options['RUN'][('end', 'end_stage')] = '-1'
    ...
    options['EXECUTION'][('re', 'resume')] = 'False'
...
def command_line_parser(thr_log_status='ERROR'):
	...
	for s_name, l_name in set([options['ALL'][x][1] for x in options['ALL']]):
		...
		if default == 'False':
#			parser.add_argument('-'+s_name, '--'+l_name, action='store_const', default=False, const=True)
			parser.add_argument('-'+s_name, '--'+l_name, action='store_const', default=default, const='True')
#			parser.add_argument('-'+s_name+'_nondefault', '--'+l_name+'_nondefault', nargs='?', default=default, action=FooAction2)
			falsedef.add(l_name)
		else:
...
#			print("no attr and not a switch", x)
			continue
...
#	print(options)
	...
	print_log(deb_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)
...
def instruction_file_parser(instruction_filename, thr_log_status='ERROR'):
	...
	if not instruction_filename:
		...
		print_log(crt_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)
...
def check_options(command_line_options, instruction_file_options, thr_log_status='ERROR'):
	...
	for x in instruction_file_options:
		if x not in def_options['ALL']:
			...
			print_log(wfl_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)
		...
		if if_tp != df_tp and def_options[def_options['ALL'][x][0]][def_options['ALL'][x][1]]:
			#print(x, def_options[def_options['ALL'][x][0]][def_options['ALL'][x][1]])
			wtp_msg = ('CRITICAL', this_name, 'in instruction file, option {0} is of type {1}, but it should be of type {2}'.format(x, if_tp, df_tp))
			print_log(wtp_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)
		else:
			f0 = def_options['ALL'][x][0]
			f1 = def_options['ALL'][x][1]
			if f0 not in options:
	...
	for x in command_line_options:
		...
		df_tp = type(def_options[def_options['ALL'][x][0]][def_options['ALL'][x][1]])
		#print(x, cl_tp, df_tp, command_line_options[x], def_options[def_options['ALL'][x][0]][def_options['ALL'][x][1]])
		if cl_tp != df_tp:
			...
			if x in instruction_file_options and command_line_options[x] != instruction_file_options[x]:
				...
				print_log(chg_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)				
			f0 = def_options['ALL'][x][0]
			f1 = def_options['ALL'][x][1]
			if f0 not in options:
				options[f0] = collections.OrderedDict()
	...
	if options['PATHS'][('sing', 'singularity')] and options['PATHS'][('container', 'singularity_container')]:
		...
		for x, y in [('ppm', 'ppm_path'), ('muscle', 'muscle_path'), ('cesymm', 'cesymm_path'), ('symd', 'symd_path'), ('quatsymm', 'quatsymm_path'), ('frtmalign', 'frtmalign_path'), ('dssp', 'dssp_path')]:
	...
	if options['RUN'][('hpc', 'run_on_hpc')]:
		if not options['RUN'][('hpcdir', 'hpc_exec_path')]:
			...
			print_log(wtp_msg)
#		elif ((not options['RUN'][('hpclsf', 'local_shared_folder')]) or not os.path.exists(options['RUN'][('hpclsf', 'local_shared_folder')])):
#			print(options['RUN'][('hpclsf', 'local_shared_folder')])
#			wtp_msg = ('CRITICAL', this_name, 'the path specified in the option hpc_exec_path does not exist. Please insert a valid absolute path')
#			print_log(wtp_msg)
...
#	locations['FSYSPATH']['src'] = os.path.dirname(os.path.abspath(__file__))
	return locations
            ...
            for y in locations['FSYSPATH']:
                if not os.path.exists(locations['FSYSPATH'][y]):
                    if options['EXECUTION'][('locp', 'location_permissive')]:
                        ...
                        #print(options['EXECUTION'][('locp', 'location_permissive')])
                    ...
                    print_log(loc_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)
        else:
            for y in locations['FSYSPATH']:
                if not os.path.exists(locations['FSYSPATH'][y]):
                    #print("MKDIR2", locations['FSYSPATH'][y])
                    os.mkdir(locations['FSYSPATH'][y])
...
def initialize_repository(main_path: str = "", instr_filename_path: str = ""):
    """
    Initialize EncoMPASS repository.

    Parameters
    ----------
    main_path : str
        Directory containing the main repository / options file.
        If empty, it is assumed that the user is running from the correct directory
        or passing a full path via the CLI.
    instr_filename_path : str
        Either:
        - empty string: auto-detect instruction file in main_path
        - relative filename: interpreted relative to main_path
        - absolute path: used as-is (takes precedence over main_path)
    """
    this_name = initialize_repository.__name__
    now_datetime = datetime.datetime.now()
    print("Initialize Encompass Repository", this_name, now_datetime)

    # Read options from command line if main_path not provided
    if not main_path:
        cl_opt = command_line_parser()
        instr_filename_path = cl_opt['instruction_filename']
        # In this branch we trust whatever the CLI provided:
        # - absolute path → used as-is
        # - relative path → interpreted relative to current working directory
    elif instr_filename_path == "":
        # No explicit instructions file name: look for one in main_path
        cl_opt = {}
        instr_filename_paths = glob.glob(os.path.join(main_path, "EncoMPASS_options_relative_to*"))
        if len(instr_filename_paths) > 1:
            crt_msg = (
                'CRITICAL',
                this_name,
                "More than one instruction file in the main path {0}.\n"
                "Please only retain the wanted one.".format(main_path)
            )
            print_log(crt_msg)
        instr_filename_path = instr_filename_paths[0]
    else:
        # main_path provided AND an instructions filename/path given explicitly
        cl_opt = {}
        if os.path.isabs(instr_filename_path):
            # User gave an absolute path; respect it and ignore main_path
            pass
        else:
            # Relative name → interpret relative to main_path (old behavior)
            instr_filename_path = os.path.join(main_path, instr_filename_path)

    print("INSTRUCTION_FILE", instr_filename_path, "\n")
    ...
    # Creates file system
    ## Find define_locations.txt in the package references
    ref_root = files("encompass.data").joinpath("reference")
    define_loc_res = ref_root.joinpath("define_locations.txt")
    with as_file(define_loc_res) as define_loc_path:
        locations = read_locations_file(str(define_loc_path), options=options)

    locations = bind_locations(locations, options, main_path=main_path)
    ## Ensure that the structure template is defined
    if not main_path:
    ...
    print("Completed", this_name, complete_time)
    return options, locations

if __name__ == "__main__":

    options, locations = initialize_repository()#main_path="/data/local/encompass/dev/argh4")
    print("Completed initializing", config.log_filename)
