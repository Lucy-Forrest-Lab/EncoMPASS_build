...
from encompass.sources import uniprot_requests

from encompass.pipeline import config  # Uses datetime



"""
def read_dfn():
    with open(os.path.dirname(os.path.abspath(__file__)) + '/debug_filename.txt') as df:
        name = df.read()
        name = name.split('\n')
    return name
...
def write_dfn(name):
    with open(os.path.dirname(os.path.abspath(__file__)) + '/debug_filename.txt', 'w') as df:
        df.write(name)
    return
"""
...
def string_decode_dict(s, thr_log_status='ERROR'):
    ...
    if not (len(s) > 1 and s[0] == "{" and s[-1] == "}"):
        ...
        print_log(fst_err)#, thr_log_status=thr_log_status, log_filename=log_filename)
    s = s[1:-1]
    if not s:
        return {}
    ...
    for c in s:
        if c == "," and not inquotes and not inbracks:
            if type(key) == type(None):
                ...
                print_log(bfe_err)#, thr_log_status=thr_log_status, log_filename=log_filename)
...
#           self.elemdictionary = dictionary
...
#       print(self.elemdictionary)
    ...
    def show_dictionary(self, t='', selfcontainer=None, quiet=False, transparency=False, deeptransparency=False, nullify=False, annotate=False):
        ...
        if len(self._dictionary) == 0 or nullify:
            ...
            # Nesting OK, but nullify WRONG
#           sc = self.elemdictionary.show_dictionary(t=t+'\t', quiet=quiet, nullify=True, transparency=transparency, deeptransparency=deeptransparency, annotate=annotate)          
            # Nullify OK, but nesting WRONG
#           print("Now parse the FD inside the FVD")
#           print(self.elemdictionary.show_dictionary(t=t+'\t', quiet=True, nullify=True, transparency=transparency, deeptransparency=True, annotate=True))
            if deeptransparency:
...
#           print("FIXEDVALDICT - SHOW_DICTIONARY", type(self.elemdictionary), self.elemdictionary)
#           fd = FixedDict({})
#           sc = fd.show_dictionary(t=t+'\t', selfcontainer=self.elemdictionary.show_dictionary(quiet=True, transparency=False), quiet=True, transparency=transparency, deeptransparency=deeptransparency, nullify=True, annotate=annotate) 
#               sc["FixedDict::<Template>"] = self.elemdictionary.show_dictionary(t=t+'\t', quiet=True, transparency=transparency, deeptransparency=deeptransparency, nullify=True, annotate=annotate)
        else:
            for k in self._dictionary:
                if not quiet:
...
#       d = self.elemdictionary.show_dictionary(t=t, quiet=quiet, nullify=True)
#       for x in d:
#           print("INTERNAL")
#           print(x)
#           print(type(d[x]))
...
class FixedDict(collections.abc.MutableMapping):
    ...
    def __steq__(self, other, selfdict={}):
        ...
        return od == sd
    def show_dictionary(self, t='', selfcontainer=None, quiet=False, transparency=False, deeptransparency=False, nullify=False, annotate=False):
#       print("FIXEDDICT - SHOW_DICTIONARY", self._dictionary)
        ...
        if type(con) == dict:
            ...
            for x in con:
                if type(con[x]) in contlist:
                    ...
                    sc[y] = con[x].show_dictionary(t=t+'\t', quiet=quiet, transparency=False, deeptransparency=deeptransparency, nullify=nullify, annotate=annotate)
                    """
                    if not (transparency or deeptransparency):
                        sc[y] = con[x]
                    else:
                        sc[y] = con[x].show_dictionary(t=t+'\t', quiet=quiet, transparency=False, deeptransparency=deeptransparency, nullify=nullify, annotate=annotate)
                    """
...
#               print(x, obj[x])
            ...
            ### WRONG!! ONLY FOR EMPTY STRUCTS
            new_key = ''.join(x.split('::')[1:])
            if deeptransparency:
                new_obj[new_key] = obj[x]
            else:
#               print(x, obj[x])
#               new_obj[new_key] = FixedList(FixedDict(obj[x]))
...
def write_checkpoint(str_data, checkpoint_file):
    transp_str_data = {}
    for pdbi in str_data:
        transp_str_data[pdbi] = str_data[pdbi].show_dictionary(quiet=True, deeptransparency=True, annotate=True)
#       print(pdbi)
#       print(transp_str_data[pdbi])
    transp_str_data_str = json.dumps(transp_str_data)
    try:
        with open(checkpoint_file, 'wb') as f:
            pickle.dump(transp_str_data_str, f)
    except Exception as e:
            print("An error occured creating:", e, checkpoint_file)
    print("created", checkpoint_file)


def fit_into_template(template_str_data_entry, data_structure, recur=0):
    """
    print("FIT INTO TEMPLATE", recur)
    print("TEMPLATE TO BE FIT")
    print(type(template_str_data_entry))
    if isinstance(template_str_data_entry, FixedDict) or isinstance(template_str_data_entry, FixedValDict) or isinstance(template_str_data_entry, FixedList):
        print(template_str_data_entry.show_dictionary(deeptransparency=True, nullify=True, annotate=True))
    else:
        print(template_str_data_entry)
    print("FITTING DATA STRUCTURE")
    print(type(data_structure))
    print(data_structure)
#   print(type(template_str_data_entry))
    """
#   print("TEMPLATE", template_str_data_entry)
    if isinstance(template_str_data_entry, FixedDict):
        if type(data_structure) != dict:
            raise TypeError
#       print("IN", template_str_data_entry.show_dictionary(quiet=True, nullify=True))
        if data_structure == {}:
#           print("EMPTY DICT DATA STRUCTURE - FIXEDDICT TEMPLATE:", template_str_data_entry)
            return FixedDict(template_str_data_entry.show_dictionary(quiet=True, nullify=True))
        else:
#           print("NONEMPTY DICT DATA STRUCTURE - FIXEDDICT TEMPLATE:", template_str_data_entry)
...
#       print("IN", template_str_data_entry.show_dictionary(quiet=True, nullify=True))
        if data_structure == {}:
#           print("EMPTY DICT DATA STRUCTURE - FIXEDVALDICT TEMPLATE:", template_str_data_entry)
#           print(template_str_data_entry.show_dictionary(quiet=True, nullify=True))
            return FixedValDict(FixedDict(template_str_data_entry.get_fdict_template()))
        else:
#           print("NONEMPTY DICT DATA STRUCTURE - FIXEDVALDICT TEMPLATE:", template_str_data_entry)
#           print(template_str_data_entry.show_dictionary(quiet=True, nullify=True))
            nulltemp = FixedValDict(FixedDict(template_str_data_entry.get_fdict_template()))
#           print("DATA STRUCTURE:", data_structure)
            for x in data_structure:
#               print("DATA STRUCTURE KEY: ", x)
                nulltemp.new(x)
#               print("TEMPL", template_str_data_entry.show_dictionary(quiet=True, deeptransparency=True))
#               print(">>>>>>> CREATING FIXED DICT INSTANCE")
...
#           print(nulltemp)
        ...
        if type(data_structure) != list:
            raise TypeError
#       print("IN", [template_str_data_entry.get_fdict_template()])
        if data_structure == []:
...
#       print("IN", template_str_data_entry, type(template_str_data_entry))
...
def write_locusts_parfile(options, locusts_filename, job_code, only_gather=False):
    text =  (
        ...
        "{0:30}\t{1}\n"
        "{2:30}\t{3}\n\n"
    ).format('run_on_hpc', options['RUN'][('hpc', 'run_on_hpc')], 'run_on_hpc_local', options['RUN'][('hpclocal', 'run_on_hpc_local')])

    if options['RUN'][('hpc', 'run_on_hpc')] or options['RUN'][('hpclocal', 'run_on_hpc_local')]:
        text += "### HPC\n"
       
        for k in ['host_name', 'partition', 'requested_nodes', 'cpus_per_node', 'local_shared_dir', 'data_transfer_protocol', 'nodewise_scratch_folder', 'nodewise_scratch_space', 'email_address', 'extra_outer_statements', 'min_stack_per_core', 'walltime']:
            if options['ALL'][k]:
    ...
    with open(locusts_filename, 'w') as outf:
        outf.write(text)
        print("Created", locusts_filename)

    return

def read_checkpoint(checkpoint_filename, template_filename):
    template_str_data_entry = define_str_data_entry(template_filename)
    try:
        with open(checkpoint_filename, 'rb') as f:
            transp_str_data_str = pickle.load(f)
    except Exception as e:
        print("Error opening", checkpoint_filename, e)
    print("Resuming from checkpoint file", checkpoint_filename)
    tmp_data = json.loads(transp_str_data_str, object_hook=object_decoder_transp)

#   print(tmp_data)
#   print(template_str_data_entry.show_dictionary(deeptransparency=True, quiet=True, annotate=True))
    str_data = {}
...
def fixedvaltest():
#   fpre = FixedDict({'questo' : '', 'quello' : FixedDict({1:'', 2:'', 3:''})})
#   print(fpre.show_dictionary())
#   fdata = {'questo' : '', 'quello' : {1:2, 2:3}}
#   print(fdata)
#   fdatayep = fit_into_template(fpre, fdata)
#   print(fdatayep.show_dictionary(deeptransparency=True, annotate=True))
#   print(fpre.show_dictionary(nullify=True))
#   print("HUUUUUU")
#   fvpre = FixedValDict({'questo' : '', 'quello' : ''})
#   print(fvpre.show_dictionary())
#   exit(1)
...
#   fvdata = FixedValDict({'A' : FixedDict({'questo' : '', 'quello' : FixedValDict({1: FixedDict({"uno" : "bo", 'due' : {}, 'tre' : []})})}), 'B' : FixedDict({'questo' : '', 'quello' : FixedValDict({'1' : FixedDict({'uno' : 'u', 'due' : {1:2}, 'tre' : [1,2,3]})})})})
...
#   pickle.dump(json.dumps(fv.show_dictionary(quiet=True, deeptransparency=True, annotate=True)), open("provauno.pkl", 'wb'))
#   print("WRITTEN ON FILE:", pickle.load(open("provauno.pkl", 'rb')))
#   tmpfv = json.loads(pickle.load(open("provauno.pkl", 'rb')), object_hook=object_decoder_transp)
#   print("AFTER LOAD", tmpfv)
#   newfv = fit_into_template(fv, tmpfv)
#   print(newfv)
    print(fv.show_dictionary(transparency=True, annotate=True))
    print("########################################")
...
def mmCIF_header_stats(stats_location, headers):
    ...
    for header in headers:
        depo_dates.append([int(x) if x else 0 for x in header['deposition_date'].split('-')])
    ...
    print(depo_dates, resolutions, experiments, biological_assemblies)
    # Deposition dates
    if len(depo_dates) > 2:
    ...
    if len(set(resolutions)) > 2:
        ...
        #nbins = 20
        binsize = 1#(max_res - min_res)/nbins
            ...
            if record_type in {"def", "loop"}:
                ...
                # Otherwise count the fields on this line
                    ...
                    for w in line.split():
                        ...
                        #print("VALS_ON_LINE", vals_on_line)
                    vals += vals_on_line
                #print("LINE", line.strip(), "VALS", vals, "NVALS", n_vals, "IVALS", len(vals))
                if len(vals) == n_vals:
                    if record_type == "def":
...
#                   if l1 == (mpart, w, len(mpart) + len(w) + 1 > linelength, a_capo, endline, (w and w[0] in ['[', '{'])):
#                       print("STOOOOOP")
#                       exit(1)
#                   l1 = (mpart, w, len(mpart) + len(w) + 1 > linelength, a_capo, endline, (w and w[0] in ['[', '{']))
#                   print(mpart, w, len(mpart) + len(w) + 1 > linelength, a_capo, endline, (w and w[0] in ['[', '{']))
...
#                           print("YEP", ipar,  w[ipar+1:].index(']'))
        ...
        for ich, ch in enumerate(str_data_pdbi['ENCOMPASS']['structure']['kchains']):
            unp_acc_list = ast.literal_eval(str_data_pdbi['ENCOMPASS']['structure']['chains'][ich]['UniProt_acc'])
            print(unp_acc_list)
            for acc, (bn, en), (strb, stre) in unp_acc_list:
                #print(bn, en)
                #print(str_data_pdbi['FROM_PDB']['pdb_id'], ch, strb, ' ', stre, ' ', acc, acc, bn+1, en+1)
...
def read_coord_from_PDB_text(text):
    print("read_coord_from_PDB_text")
    print(text.split('\n')[-20:])
...
#            if atname == 'CA':
#                cXlegend[ch_name].append((ch_name, ((r, ichar), resname)))
#                cX[ch_name].append(coordinates)

#    for ch_name in cX:
#        cX[ch_name] = np.array(cX[ch_name])
...
#            print("CLASH", ch_name1, r1, ch_name2, r2, np.amin(clash_d))
    ...
    for ch_name in coords:
        ...
        for i, resid in [(ix, x) for ix, x in enumerate(resid_list)][1:]: # resid is (<int>resid, <str>inscode)
            ...
            if resid_d > 1:
                #print(resid, cX[ch_name][i-1])
                #print(resid, cX[ch_name][i])
...
def make_DSSP(dssp_path, coord_filename, dssp_out_filename, pdbi, c_dict={}, cache_coord_filename='', opm_check=True, leave_original_dssp=True, container=False):
    # INCOMPLETE: options opm_check and leave_original_dssp not coded yet! opm_check should check the consistency with opm
    # Run DSSP
    fnull = open("/dev/null", "w")
#   print(dssp_path, '-i', coord_filename, '-o', dssp_out_filename)
    dssp_clist = dssp_path.split()
#   print(dssp_clist + ['-i', coord_filename, '-o', dssp_out_filename])
    ...
    with open(dssp_out_filename, 'r') as dssp_file:
        for line in dssp_file:
            ...
            if start_next and fields[1] != '!':
                ...
                # T ("Turns", or disorderd helices with hbond signal) are considered for joining segments
                elif line[16].strip() == "T":
                    if line[11] not in Tres:
                        Tres[line[11]] = set()
                    resid = (int(line[5:10].strip()), line[10].strip())
                    Tres[line[11]].add(resid)
#                   print(line[11], resid, "IN TRES")
...
#    print("Chooses best Altloc and gets rid of the others")
    ...
    # Biopython readability suppressed: we have to check it every time
    """
    # TO DO where is biopython used now?
    # Checks if structure can be read by Biopython. The non-readability only generates a warning: the structure can go on even if it's not readable by Biopython
    if scan and biopython_compatible:
        tmp_biopython_filename = locations['FSYSPATH']['TMPcoords'] + '{0}_biopy_tmp.pdb'.format(struct_filename.split('/')[-1].split('.')[-2])
        write_biopython_input('', tmp_biopython_filename, text=text)
        parser = PDBParser(PERMISSIVE=True, QUIET=True, get_header=False)
        try:
            structure = parser.get_structure('self', tmp_biopython_filename)
            chains_with_coords = [x.id for x in structure[0].get_chains()]
            biopython_readable = True
        except:
            biopython_readable = False
            chains_with_coords = []

#       print(struct_filename, sorted(chains_with_coords), sorted(chain_list), biopython_readable)
        if biopython_readable and sorted(chains_with_coords) != sorted(chain_list):
            biopython_readable = False
    """
...
def convert_coords_to_enc(locations, str_data, checkpoint_filename):

    pdb_atom_dict_location = locations['SYSFILES']['pdbatomdict']
    for pdbi in str_data:
        ...
        write_ENC(str_data[pdbi], c_dict_pdbi, locations['FSYSPATH']['ENCpdbs'] + pdbi + '_enc.pdb',
                  locations['SYSFILES']['ignoredpasscodes'], pdb_atom_dict_location, only_pass=only_pass)
        write_ENC(str_data[pdbi], c_dict_pdbi, str_data[pdbi]['ENCOMPASS']['coordinate_file'],
                  locations['SYSFILES']['ignoredpasscodes'], pdb_atom_dict_location, only_pass=only_pass)
...
def uncompressed_download(url, local_filename):
    ...
    try:
        ...
        #print(response) # TODO response.text
    except:
        return False

    local_file = open(local_filename, 'wb')
    shutil.copyfileobj(response, local_file) # TODO response.raw = a file-like object
    local_file.close()
    return True
...
def iterate_download(url, local_filename, check="", gzipped=False, thr_log_status="ERROR"):
    ...
    while (error_found or not downloaded):
        if iterations >= max_r:
            return False
        if iterations > 0:
            time.sleep(iterations**2)
            try_msg = ('NOTICE', this_name, "Connect to {0} \t Try number: {1}".format(url, iterations))
            print_log(try_msg)#, thr_log_status=thr_log_status, log_filename=log_filename)
        if gzipped:
            downloaded = download_url_file(url, local_filename, zipped=True)
        else:
            downloaded = download_url_file(url, local_filename, zipped=False)
        if downloaded:
            with codecs.open(local_filename, "r",encoding='utf-8', errors='ignore') as local_file:
                for line in local_file:
                    if "Error: Could not connect to database. Please try again later." in line:
                        error_found = True
            if not error_found and check == "bio.mmcif":
                try:
                    parser = MMCIFParser()
                    structure = parser.get_structure("try", local_file)
                except:
                    error_found = True
            elif not error_found and check == "bio.pdb":
                try:
                    parser = PDBParser()
                    structure = parser.get_structure("try", local_file)
                except:
...
def download_url_file(url, local_filename, zipped=False):
    """
    Downloads a file from a given URL and optionally decompresses it if it's a GZIP file.

    Parameters:
        url (str): The URL of the file to download.
        local_filename (str): The local path where the file should be saved.
        zipped (bool): If True, the file is assumed to be GZIP-compressed and will be decompressed.
                       If False, the file will be downloaded as-is.
    ...
    try:
        ...
        if response.getcode() != 200:
            print(f"Error: Received HTTP {response.getcode()} from {url}")
            return False
    except HTTPError as e:
        print(f"HTTP Error: {e.code} - {e.reason} while accessing {url}")
        return False
    except URLError as e:
        print(f"URL Error: {e.reason} while accessing {url}")
        return False
    except Exception as e:
    ...
    try:
        ...
        print(f"File successfully downloaded and extracted to {local_filename}")
        return True
...
#       reiterated_simple_download('https://files.rcsb.org/download/{0}.pdb'.format(pdbi.upper()), struct_filename)
        #print(parse_PDB_coords(struct_filename, test_folder + pdbi + '_scanned.pdb', scan_data=[{'PARAMETERS' : {('', 'hole_thr') : 100, ('', 'clash_distance') : 3}}, {}, {'PASSPORT' : []}], scan=True))
...
def reduce_chains(str_data):
    ...
    for pdbi in str_data:
        ...
        for secpdbi in [x['pdbid'] for x in str_data[pdbi]['FROM_OPM']['secondary_representations']]:
            ...
            #print('TEST', pdbi, secpdbi, test)
            #print(str_data[pdbi]['ENCOMPASS']['structure']['kchains'])
            #print(str_data[secpdbi]['ENCOMPASS']['structure']['kchains'])
            if test:
                ...
                if str_data[pdbi]['ENCOMPASS']['structure']['redundant_chains'] == str_data[secpdbi]['ENCOMPASS']['structure']['redundant_chains']:
                    #print('CHECKRED', pdbi, secpdbi, True)
                    lock_set.add(pdbi)
                    rmsec_set.add(secpdbi)
                    rm_set |= {secpdbi + "_" + k for k in str_data[secpdbi]['ENCOMPASS']['structure']['kchains']}
                else:
                    pass
                    #print('CHECKRED', pdbi, secpdbi, False, str_data[pdbi]['ENCOMPASS']['structure']['redundant_chains'], str_data[secpdbi]['ENCOMPASS']['structure']['redundant_chains'])

    #print(" ".join(sorted(list(all_set - rm_set))))
    #print(len(all_set), len(rm_set))
...
def colorblind_palette(n=0):
    from encompass.utils import cbf_colors
    if n<9:
        l = [
            "cbf_orange", 
            "cbf_sky_blue",
...
"""
def approx_equality_transfs(t1, t2, epsilon):
    #DISMISSED
    #Approximate comparison between translvecs
    if len(t1) != len(t2):
        return False

    for it1 in range(len(t1)):
        if t1[it1]['chains'] != t2[it1]['chains']:
            return False
        for i in range(3):
            if abs(t1[it1]['translvec'][i] - t2[it1]['translvec'][i]) > epsilon:
                return False
            for j in range(3):
                if abs(t1[it1]['rotmtx'][i][j] - t2[it1]['rotmtx'][i][j]) > epsilon:
                    return False
...
def read_OPM_representation_chart(OPMreprchart):
    """Reads the representation chart, which relates so-called secondary
    entries that are identical to a primary entry, for which most info is
    stored. This code ASSUMES the list is ORDERED and thus
    that secondary entries refer to the preceding primary entry
    """
    ids = []
    secrep_d = {}
...
def make_structurewise_table(str_data, filename):
    ...
    print("Created tsv table with details for each structure", filename)
...
def find_redundant_chains(locations, str_data):
    ...
    for pdbi in str_data:
        ...
        str_data[pdbi]['ENCOMPASS']['structure']['redundant_chains'] = {}
        #enc_eq = str_data[pdbi]['ENCOMPASS']['structure']['redundant_chains']
        for ich1, ch1 in enumerate(allch):
            ...
            for ich2, ch2 in [(ix, x) for ix, x in enumerate(allch)][ich1+1:]:
                #if ch1 in enc_eq and ch2 in enc_eq and enc_eq[ch2] == enc_eq[ch1]:
                    #print(ch2, "excl")
                    #continue
            ...
            partial_lists = sorted(merge(samech, sorted_lists=True))
...
if __name__ == "__main__":
    ...
    test_read_oper_expression()
#    test_mmCIF_format_parser()
#    test_header_parsers()
#    test_coord_parsers()
    test_roe()
