    ...
    if output_fmt == 'tab':
        ...
        for r in results['results']:
            if 'sequence' in r['to']:
                ...
                #d[r['to']['primaryAccession']] = r['to']['sequence']['value']
    return d
...
def assign_uniprot_acc(locations, str_data, eq):
    ...
    #  that can also accommodate for starting and ending points of the struct chain
    uniprot_codes = set()
    ...
    print('\n\n\n')
    print("UNIPROT DICT")
    for pdbi in sorted(uniprot_dict):
        print(pdbi, uniprot_dict[pdbi])
    print('\n\n\n')
    """
...
#                        print(pdbi, ch, name, "BIOPYTHON ERROR: null result")
                        status[pdbi+"_"+ch] = 'BIOPYTHON ERROR - NULL'
                        continue
                except:
                    if name not in unp_seqs:
#                        print(pdbi, ch, name, "UNIPROT ERROR: sequence not retrieved from UniProt")
                        status[pdbi+"_"+ch] = 'UNIPROT ERROR - NO SEQUENCE'
                    else:
                        status[pdbi+"_"+ch] = 'BIOPYTHON ERROR - FAILED'
#                        print(pdbi, ch, name, "BIOPYTHON ERROR: call failed")
                    continue
                if result:
                    aln = result[0]#list(zip(result[0][0], result[0][1]))
                ...
                if seqid > bestid:
                    ...
                    bestaln = aln
                    bestname = name
                    #print(pdbi, strb, stre-1, len(seq), len(resids))
                    #print(resids[strb][0])
                    #print(resids[stre-1][0])
                    if strb == -1 or stre == -1:
                        bestextr = (-1, -1)
                    else:
            ...
            if len(ENCchain['TM_regions']['TM_regions_extrema'])>0\
                ...
                for pdbi_ch2 in eq[pdbi_ch1]:
                    ...
                    if ENCchain2['UniProt_acc']:
                        ...
                        #print("FOUND REPLACEMENT", pdbi_ch1, pdbi_ch2)
                        break
...
def aln_and_seqid(seq1, seq2, sn1, sn2, muscle_path='', cache_path=''):
    """Calculates pairwise alignment and sequence identity
    Low-level function: independent of EncDS
    """

### WARNING ###
# The Biopython package is not supported anymore for this function
    ...
    if muscle_path:
        ...
        while (not os.path.exists(muscle_out_filename)) and i<3:
            ...
            try:
                subprocess.check_call(muscle_plist + ['-align', muscle_in_filename, '-output', muscle_out_filename], stdout=open(muscle_stdout, 'w'), stderr=open(muscle_stderr, 'w'))
            except:
...
def find_copies_of_chains(coord_dict_pdbi, analysis_pdbi, pdbi, pdb_data_pdbi={}, pdb_bioassembly_group=-1): #, thr_log_status="ERROR"):
    ...
    #print("CHAINS TO CONSIDER", chains_to_consider)
    ...
    # UNFINISHED: if bioassembly, and if bioassembly and chains with coords were not in agreement, make a warning!
...
def generate_full_structure(str_data_pdbi, pdbi, coords={}, bioassembly=0): #, thr_log_status="ERROR"):
    ...
    #print("GENFSTR", pdbi, [(x,len(new_coords[x])) for x in new_coords])
...
def calc_rotation_axis(rotmtx):
    ...
    if np.linalg.norm(axis_of_rotation) > config.epsilon:
        ...
        #print(np.linalg.norm(axis_of_rotation))
...
def select_biological_assembly(data):
    ...
    print("Selecting a biological assembly")
    ...
    #           print("CONSENSUS", pdbi, [x['pdbid'] for x in str_data[pdbi]['FROM_OPM']['secondary_representations'] if x['pdbid'] in str_data], maxi, multiplicity)
                val = invsigd[maxi][0]
    ...
    for pdbi in str_data:
        ...
        # BUT it gets checked for consistency of copies of chains. If it has the same number of copies of all chains
        # as the one suggested by PDB, ok, otherwise
...
#               print(c_dict['COORDS'][ch][r][at], new_c_dict['COORDS'][ch][r][at])
...
# QUI
# MODIFICA E FAI FUNZIONARE E METTI STATS
def build_by_comparison(data):
    this_name = build_by_comparison.__name__
    # INCOMPLETE: code the OFFLINE procedure
    ...
    if downloaded:
        PDBTM_dict, PDBTM_report = parse_PDB_coords(str_data_pdbi['FROM_PDBTM']['coordinate_file'], scan=True, options=options) #cache_filename=str_data_pdbi['FROM_PDBTM']['cache_coordinate_file'], with_UNK=options['PARAMETERS'][('', 'with_UNK')])
        if PDBTM_dict:
            ...
            str_data_pdbi['PASSPORT'].append(passport_entry('{0}_rel'.format(this_name), pdbi, "The structural and/or format inconsistencies of this entry could not be resolved with the help of the coordinate file from the PDBTM database. This entry is scheduled for deletion"))
...
def decision(locations, str_data):
    ...
    for pdbi in str_data:
        ...
        # If not, if PDB ok, go on and add to PPM
            ...
            str_data[pdbi]['status'].append('run_in_PPM')
        # If not, if PDBTM ok, go on and add to PPM
#       elif 'pdbtm_approved' in str_data[pdbi]['status'] and 'pdbtm_eliminated' not in str_data[pdbi]['status']: #and 'pdbtm_monitored' not in str_data[pdbi]['status']:
#           str_data[pdbi]['present_cache'] = str_data[pdbi]['FROM_PDBTM']['cache_coordinate_file']
            """
            if 'pdbtm_monitored' not in str_data[pdbi]['status']:
                c_dict_pdbi = retrieve_coords_from_CIF(str_data[pdbi]['present_cache'])
                str_data[pdbi]['PASSPORT'].append(passport_entry(this_name+'_pdbtmok', pdbi, "This structure reflects its description in the PDBTM database, and has passed all structural checks"))
                write_ENC(str_data[pdbi], c_dict_pdbi, str_data[pdbi]['ENCOMPASS']['coordinate_file'], locations['SYSFILES']['ignoredpasscodes'], LOC_pad)
                n_pdbtm += 1
                print_log(('NOTICE', this_name, "pdbtm {0}".format(pdbi)))
            else:
            """
#           run_in_PPM.add(pdbi)
#           n_pdbtm_ppm += 1
...
#           str_data[pdbi]['PASSPORT'].append(passport_entry(this_name+'_pdbtminppm', pdbi, "This structure reflects its description in the PDBTM database, and will be analyzed with the PPM software in order to insert it in a model lipid bilayer"))
#           str_data[pdbi]['status'].append('run_in_PPM')
        elif os.path.exists(str_data[pdbi]['FROM_OPM']['cache_coordinate_file']) and\
                (   'virus' in str_data[pdbi]['FROM_OPM']['superfamily']['name'].lower() or 'viral' in str_data[pdbi]['FROM_OPM']['superfamily']['name'].lower()\
...
#           print(pdbi)
            #str_data[pdbi]['PASSPORT'].show_list()
...
def parallel_generate_full_structure(options, locations, str_data,
    ...
    for pdbi in str_data:
        ...
        # Version for update processes (now muted)
        """
        # If it is a monitored or already unsafe structure check if it has copies of chains with coordinates, and decide what to record in the unsafe set of chains
        # Warning: no structure is deemed unsafe at the first use, this is only for database completions...
        elif 'pdb_unsafe' in status: #or 'pdb_monitored' in status:
            # REMOVED find_copies_of_chains FROM HERE AND TAKE IT FROM THE str_data: pdb_unsafe cannot occur in a first run
            copies_ch = str_data[pdbi]['FROM_PDB']['analysis']['chain_copies'][0]['list_copies']
            # If it is monitored and has copies of chains, take one chain per copy and try to make a bioassembly out of it
            # Otherwise, take all chains with coordinates (there will be only sublists of length 1)
            biological_assembly['UNSAFE'][pdbi] = [sorted(x)[0] for x in copies_ch]
        # If it is already deemed as safe, put it in safe. Warning: no structure is deemed safe at the first use, this is only for database completions...
        elif 'pdb_safe' in status:
            biological_assembly['SAFE'][pdbi] = str_data[pdbi]
        # Othewise, it must be judged
        else:
            if pdbi in str_set:
                if options['PARAMETERS'][('', 'consensus_signature_PDB')]: # Option for grouping judgements over OPM representation groups 
                    pre_data.append(pdbi)
    ...
    for pdbi in biological_assembly['UNSAFE']:
        ...
        if 'opm_eliminated' not in str_data[pdbi]['status']:
            ...
            if downloaded:
                ...
                PDBTM_dict, PDBTM_report = parse_PDB_coords(str_data[pdbi]['FROM_PDBTM']['coordinate_file'], scan=True, options=options) #cache_filename=str_data_pdbi['FROM_PDBTM']['cache_coordinate_file'], with_UNK=options['PARAMETERS'][('', 'with_UNK')])
                if PDBTM_dict:
                    ...
                    #PDBTM_dict, str_data[pdbi] = PDBTM_to_ENC(PDBTM_dict, str_data[pdbi], pdbi, cache_filename=str_data[pdbi]['FROM_PDBTM']['cache_coordinate_file'])
                else:
    ...
    """
    #Cyc le B: remedy unsafe set
    # Step 1b: prepare independent inputs
    data = []
### ATTENZIONE A QUESTA CONDIZIONE!!! UNICHE STRUTTURE CHE HANNO BISGOGNO DI PDBTM SONO LE UNSAFE?
    for pdbi in biological_assembly['UNSAFE']:
        if generate_only and struct not in generate_only:
            continue
        data.append((options, locations, str_data[pdbi], pdbi))

    # For the moment, np=1 here, because there is a wget inside
    #numproc = int(options['RUN'][('np', 'number_of_processors')])
    numproc = 1

    pool_outputs = []
    for d in data:
        o = build_by_comparison(d)
        pool_outputs.append(o)

    # Step 2b: run multiprocess
#   pool = multiprocessing.Pool(processes=numproc)
#   pool_outputs = pool.map(build_by_comparison, data)
#   pool.close()
#   pool.join
    
    # Step 3b: collect outputs
    # The routine can also delete entries in OPM_data, but we have to retain the results of the first routine
    for s in pool_outputs:
        if not s:
            continue
        strdata, structs = s
        for pdbi in structs:
            if pdbi in strdata:
                str_data[pdbi] = strdata[pdbi]
    # At this point, all full structures have been generated, either because they were safe (cycle A), or because they were unsafe and remedied (cycle B)
    # or because they were taken from the PDBTM (cycle A or B).
    """
...
def check_PDB_biomatrix_errors(options, equivalent_chain_sets, coords, pdbi, thr_log_status="ERROR"):
    ...
    if detached_units:
        ...
        for du in detached_units:
            ...
            error = True
### PROBLEMS
# How about those chains having no biomatrix (2atk, visibly part of a KscA channel)?
    # For those, use the PDBTM procedure: BLAST search at 90% among those PDB having a >0.9 seqid,
    # then align it with one chain, then multiply that matrix with the target matrix, then produce the chain.
    # Put this in generate_full_structure! Reconsider it here, maybe.
# How about those complexes showing no bad biomatrix but only because the damage was already done internally (4r0c)?
    # Here, identify if there are identical chains. Superpose them. Is there a big translation? Exclude.
###
    if not error:
...
def run_MUSCLE(options, locations, str_data, str_set, only_analysis=False):
            ...
            for i2, pc2 in list(enumerate(alpha))[i1+1:]:
                if (pc1, pc2) not in mtx_d:
                    ...
                    print_log((
                        "ERROR",
                        this_name,
                        ("Output number {0} of job {1} lacks the comparison "
                            "between {2} and {3}")\
    ...
    if options['PATHS'][('sing', 'singularity')] and options['PATHS'][('container', 'singularity_container')]:
        ...
        print("Running singularity", muscle_path)
    else:
        if options['RUN'][('hpc', 'run_on_hpc')]:
            muscle_path = options['PATHS'][('hpcmuscle', 'hpc_muscle_path')]
            print("Running on hpc using", muscle_path, options['RUN'][('hpc', 'run_on_hpc')])
        else:
            muscle_path = options['PATHS'][('muscle', 'muscle_path')]
            print("Running locally using", muscle_path)

    for db in ['PDB', 'OPM']:
        # Instruct locusts
        batch_job_code = options['RUN'][('code', 'main_code')] + "_" + db + "_MUSCLE"
        ...
        if not only_analysis:
            ...
            print("Will calculate sequence identity using a copy of", seqid_py)
...
#            str_set = set()
            tmplist = []
            spf = ""
            print("Creating muscle inputs for each entry and its pairs")
            for pdbi in sorted(str_set):
                if len(tmplist) == 10:
                    pair_name = tmplist[0]+"-"+tmplist[-1]
#                    str_set.add(pair_name)
                    tmplist = []
                    seq_pairs_filename = input_dir + "{0}_seqpairs.txt".format(pair_name)
                    with open(seq_pairs_filename, 'w') as spff:
                        spff.write(spf)
                    command_filename = input_dir + "{0}_cmd.sh".format(pair_name)
...
#                str_set.add(pair_name)
                seq_pairs_filename = input_dir + "{0}_seqpairs.txt".format(pair_name)
                #print("HERE2", seq_pairs_filename)
                #print(tmplist)
                #print(spf)
                with open(seq_pairs_filename, 'w') as spff:
                    spff.write(spf)
        ...
        if not os.path.exists(outlog):
            print(f"The output '{outlog}' does not exist. locusts.swarm.launch probably failed")
            sys.exit(1)
    ...
    #str_data['1a0s']['FROM_PDB']['analysis'].show_dictionary()
    #str_data['1a0s']['FROM_OPM']['analysis'].show_dictionary()
...
def run_PPM(options, locations, str_set, str_data, only_analysis=False):
    ...
    print("Now launching PPM jobs", this_name)
    ...
    if str_set and not only_analysis:
        ...
        if options['PATHS'][('sing', 'singularity')] and options['PATHS'][('container', 'singularity_container')]:
            ...
            elif options['RUN'][('hpclocal', 'run_on_hpc_local')]:
                cmd = "module load singularity; export SINGULARITY_BINDPATH='/data/Encompass/,/vf/users/Encompass/'; singularity exec " + options['ALL'][('hpccontainer')] + ' cp  /EncoMPASS/ppm/res.lib ' + input_dir
            else:
        ...
        p = subprocess.Popen(cmd, shell=True, stdout=open('/dev/null', 'w'), stderr=open('/dev/null', 'w'))
        print("COMMAND CP:", cmd)
        p.wait()
...
def define_TM_regions(options, locations, coord_filename, str_data_pdbi, pdbi, c_dict={}, cache_coord_filename='', span=None):
    ...
    print(f"copy {coord_filename} to {tmp_coord_filename}")
...
##            print(ch, "CONSIDER", dsegp1, dsegp2)
...
#                            print("REENTRANT!")
                if to_be_merged:
##                    print("SEGMENTS MERGED:", dsegp1, dsegp2)
                    if jump_next:
                        added_seg = []
...
#        print("DSSP SEGMENTS REFINED", dssp_segments_refined)
        linkers = []
        ...
        #print(pdbi, "DSSP SEGMENTS", new_dssp_segments_refined)
        #print(pdbi, "LINKERS", linkers)
        #print(pdbi, len(new_dssp_segments_refined), len(linkers))
...
#            print(pdbi, ch, ndseg, flush=True)#, ndseg, dseg, flush=True)
...
#                print(pdbi, ch, dseg[1+rn], c_dict['COORDS'][ch][dseg[1+rn]]['CA'][2], lim_inf+(lim_sup-lim_inf)/2, lim_sup, (lim_sup-lim_inf)/2, dist_from_0)
                #print(pdbi, ch, dseg[1+rn], dist_from_0)
                if dist_from_0 < 1:
                    TE_weight += 1
...
#                print(ndseg+1, len(linkers))
                for r in linkers[ndseg+1]: # There will always be one more linker: the (maybe empty) cterminal. If it is not there, the sequence does not have segs
                    dist_from_0_sign = (c_dict['COORDS'][ch][r]['CA'][2] - (lim_sup+lim_inf)/2)/((lim_sup-lim_inf)/2)
                    #print("LINKERAFTER", r, dist_from_0_sign)
...
#            print("TE:", TM_weight, [dssp_to_ab[dseg[0]]] + dseg[1:], first_tmres, last_tmres, len_tmreg, len_ssintm)
...
#   out_dir = str_data_pdbi['present_cache']
    if is_ciff:
    ...
    #print(pdbi, is_ciff, str_filename)
...
def structure_sorter(options, locations, str_data):
    """ extracts sequence, number of chains etc;
    splits into chains; writes/creates the encompass coordinate file with the selected version for that entry
    looks through all the decisions & copies coordinate file into folder
    """
...
#    pdbi_c_2_acc_name = assign_uniprot_acc(locations, str_data)
#    pdbi_c_2_acc_name = ""
    ...
    for pdbi in str_data:
        ...
        print(pdbi, str_data[pdbi]['ENCOMPASS']['coordinate_file'])
        ...
        for ich, ch in enumerate(str_data[pdbi]['ENCOMPASS']['structure']['kchains']):
            ...
            #print("TES_LINKERS", len(topological_elements[ch]), len(len_linkers))
            for te, st_tmr in zip(topological_elements[ch], stats["STATS_TM_REGIONS"][ch][1:]):
                ...
                print(pdbi, ch, tei, tm_weight)
                print("LINKER", ilinker, len_linkers[ilinker])
                print("TE", len(te_segid), te_segid)
                if tm_weight > 0:
                    tmi += 1
                    is_tm = True
...
"""
def assign_uniprot_acc(locations, str_data):
    unp_seqs = {}
    unp_name2acc = {}
    with open(locations['SYSFILES']['uniprot_all']) as f:
        for line in f:
            if line.startswith(">"):
                sig = line.split()[0]
                _, acc, name = sig.split("|")
                unp_seqs[acc] = ""
                unp_name2acc[name] = acc
            else:
                unp_seqs[acc] += line.strip()

    aligner = init_pairwise_alignment()

    pdbi_c_2_acc_name = {}
    for pdbi in str_data:
        pdbi_c_2_acc_name[pdbi] = {}
        for ich, ch in enumerate(str_data[pdbi]['ENCOMPASS']['structure']['kchains']):
            seq = str_data[pdbi]['ENCOMPASS']['structure']['chains'][ich]['sequence']
            seqid_max = 0
            right_acc_name = []
            for name in str_data[pdbi]['FROM_OPM']['uniprotcodes']:
                if name not in unp_name2acc:
                    continue
                unp_seq = unp_seqs[unp_name2acc[name]]
                try:
                    result = aligner.align(seq, unp_seq)
                except:
                    print("BIOPYTHON ERROR")
                    print(seq)
                    print(unp_seq)
                    continue
                if result:
                    aln = result[0]#list(zip(result[0][0], result[0][1]))
                    norm = sum([x[1] - x[0] for x in  aln.aligned[0]])
                    seqid = 0
                    if norm != 0:
                        for i in range(len(aln.aligned[0])):
                            ix, iy = aln.aligned[0][i], aln.aligned[1][i]
                            for j in range(ix[1] - ix[0]):
                                if seq[ix[0]+j] == unp_seq[iy[0]+j]:
                                    seqid += 1
                        seqid /= norm
                if seqid > seqid_max:
                    right_acc_name = [unp_name2acc[name], name]
        pdbi_c_2_acc_name[pdbi][ch] = right_acc_name
    return pdbi_c_2_acc_name
"""
...
if __name__ == "__main__":
    ...
    print("At stage of structure sorter, post insertion analysis, find redundant chains and assign uniprot accessions")
    ...
    print("Starting post_insertion_analysis")
    str_data = post_insertion_analysis(options, locations, str_data)
    str_data = read_checkpoint(locations['FSYSPATH']['cache'] + 'str_data_5.pkl', locations['SYSFILES']['data_structure_template'])
    print("Starting to find redundant chains")
    str_data, unique_chains = find_redundant_chains(locations, str_data)
    print("Starting to assign uniprot accessions")
    str_data = assign_uniprot_acc(locations, str_data, unique_chains)

    print("Incorporating coordinates into data structure")
    LOC_pad = locations['SYSFILES']['pdbatomdict']
    for pdbi in str_data:
        if 'eliminated' in str_data[pdbi]['status']:
            only_pass = True
    ...
    print("Creating structure-wise table")
    make_structurewise_table(str_data, locations['SYSFILES']['structurewise_table'])
