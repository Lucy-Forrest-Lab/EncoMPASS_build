    ...
    for i, pdbi, is_temp in sorted(entries, key=lambda x:x[1]):
        ...
        if OPT_from_repo and os.path.exists(repofilename):
            ...
            if ((not OPT_offline) and
                ...
                #print("Download json file from OPM:", i)
                res = requests.get(
...
def scan_PDB(options, locations, str_data={}, pdb_list=[], use_pkl=False, thr_log_status='ERROR'):
    ...
    for ibatch, batch in enumerate(batches):
        ...
        for pdbi in batch:
            ...
            if options['RUN'][('mmcifmirror', 'mmcif_local_mirror')]:
            ...
            elif options['EXECUTION'][('re', 'resume')] and os.path.exists(mmcif_filename) and os.path.exists(pdb_filename):
                pass
            else:
                print("Download", mmcif_filename)
                download_result = iterate_download(
                    f"{locations['EXTERNAL']['pdbdb_mmcif']}/{pdbi.lower()[1:3]}/{pdbi.lower()}.cif.gz",
                ...
                print("Download", pdb_filename)
                download_result_2 = iterate_download(
                    f"{locations['EXTERNAL']['pdbdb_pdb']}/{pdbi.lower()[1:3]}/pdb{pdbi.lower()}.ent.gz",
            ...
            parser = MMCIFParser() # TO DO check functional
            ...
            print(pdbi, mmcif_filename, pdb_filename, mmcif_ok, pdb_ok)

            parser = PDBParser() # TO DO - check functional
    ...
    for pdbi in pdb_list:
        ...
        # TO DO: database references: from where does the sequence come from?
        # Polymers
        new2oldch = {}
        polychains = set()
        alreadythere = set()
...
#    clash_frac_thr = options['PARAMETERS'][('', 'clash_frac_thr')]
    ...
    # Check fraction of clashes

    # Check biopython compatibility
    """
    if not report['biopython_readable']:
        str_data_pdbi['PASSPORT'].append(passport_entry(this_name+'_no_biopy', pdbi, 'The Biopython package could not read the coordinate file from the {0} database. This could cause a few limitations on the analyses that can be performed'.format(prov.upper())))
        str_data_pdbi['status'].append(no_biopy)
    """
...
def check_whole_structures(data):
    ...
    for pdbi in sorted([k for k in str_data]):
        ...
        print(pdbi, "PARSE", str_data[pdbi]['FROM_PDB']['coordinate_file'])
        if PDB_dict and 'COORDS' in PDB_dict and PDB_dict['COORDS']:
            ...
            print(pdbi, "WRITE", str_data[pdbi]['FROM_PDB']['cache_coordinate_file'])
        ...
        if 'eliminated' not in str_data[pdbi]['status'] and 'opm_eliminated' not in str_data[pdbi]['status']:
            ...
            # UNCLEAR piece of code: why weren't these checks performed before?
            pdbswitch = False
...
#                str_data[pdbi]['ENCOMPASS']['coordinate_file'] = str_data[pdbi]['FROM_OPM']['coordinate_file']
        ...
        if 'eliminated' not in str_data[pdbi]['status'] and 'pdb_eliminated' not in str_data[pdbi]['status']:
            ...
            # UNFINISHED piece of code: these two options should signal to the build_from functions in complete_information!!
            else:
    ...
    if options['RUN'][('np', 'number_of_processors')] > 1:
        ...
        for s in pool_outputs:
           ...
           if pdbi in str_dict:
               ...
               #new_str_data[pdbi] = copy.deepcopy(str_dict[pdbi])
        #str_data = copy.deepcopy(new_str_data)
    else:
...
def combine_sources(options, locations, str_data):
    ...
    print("Start combining data from source databases", this_name)
    ...
    for pdbi in sorted(str_data):
        print(pdbi, "WRITE IN EncoMPASS_structures/")
        if options['EXECUTION'][('debug', 'debug')]:
            print(pdbi)
        # WHY???
...
def test_for_redundancy():
    from ..pipeline.initialize_repository import initialize_repository
