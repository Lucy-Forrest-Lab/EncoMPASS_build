...
def transfer_exec(template, target, locations, suffix, frtm_exec_path="/EncoMPASS/frtmalign/frtmalign", run_id="0000"):
	...
	#print(template_pdb,  symm_dic[template_pdb][symm_dic[template_pdb]['selected']['source']].keys())
	...
	if os.path.isfile(frtmaln_file):
		with open(frtmaln_file, "r") as f:
			if f"INIT {template} {target}" not in f.read():
				#raise SystemExit(f"INIT\t{template}\t{target} not in {frtmaln_file}.")
	...
	for i, s in enumerate(sel.strip(";").split(";")):
		#template_symm_data_dir = symm_dic[template_pdb][s]['files_dir_path']
...
if __name__ == "__main__":
	...
	#container = "/data/local/encompass/dev/Ubuntu1804-EncoMPASS.sif"
	#frtmpath = "singularity exec " + container + "  /EncoMPASS/frtmalign/frtmalign"
	#print(frtmpath)
	#template_chain = "5e9s_A"
	#target_chain = "2nwx_A"
	#frtmaln(wkdir, template, target, frtmpath)
