# Name: results_selection_v3.py
# Author: Antoniya A. Aleksandrova
# Language: Python 3.5
# Date: 1 Nov 2017
# Updated: 12 Aug 2018
# Description: Create an xml file that lists the data from CE-Symm and SymD that should be
#              displayed in the second symmetry section of the website EncoMPASS
#              v2: changed criteria for bug fix to allow for CE-Symm OR SymD
#              v3: 3dmol implemented; loop structure for chain ordering; only quaternary symmetry is reported for whole complex/pdb; generates PyMOL script for superposition of repeats;
# Usage: python results_selection_v3.py <pdb_code>
import shutil
import glob
...
def results_selection(locations, options, pdb_suff, inputf, run_id="0000", want_image = 1, ray = 0):
    ...
    #pymol = "/v/apps/pymol/1.8.4.2/pymol -c"

    #pymol = "/usr/local/apps/pymol/2.5.0/pymol/bin/pymol -c"
    pymol = "pymol -c"
    muscle_exe = "muscle"
    ...
    xml_dir = locations['FSYSPATH']['symmtemp'] + "webarchive/analysis/"
    ...
    #if os.stat(out1_name).st_size == 0:
    #    out1.write("#PDB\tTM-Chains\tNum-Repeats\tSymm-Levels\tSymm-Order\tTopology\tSymm-Type\tAngle\tTranslation\t"
    #               "TM-Score\tRMSD\tRep-Length\tAligned-Length\tSize\tCoverage\tSymm-Method\tStatus\n")

    #if os.stat(out2_name).st_size == 0:
    #    out2.write("#PDB\tSymm-Method\tTM-Chains\tNum-Repeats\tSymm-Levels\tSymm-Order\tTopology\tSymm-Type\t"
    #               "Angle\tTranslation\tTM-Score\tRMSD\tRep-Length\tAligned-Length\tSize\tCoverage\n")


    #if os.stat(out3_name).st_size == 0:
    #    out3.write("#PDB\tChange\n")
    out3.write(inputf + "\t")
    ...
    # quat_sym=symd_quat_str(symd_out_dir,inputf,oriented,chains) #Check if SymD's alignment indicates quaternary or internal symmetry
    # symmlist[inputf]['symd']['quaternary']=quat_sym
    ...
    if os.path.isfile(locations['FSYSPATH']['quatsymm'] + inputf + "/" + inputf + "_qsymm_enc.pkl"):
        ...
        if symmlist[inputf][sel]['symmetry_levels'] != 'na':
    ...
    # if sel!='cesymm' and cesymm_quat_symm==1:
    #    out3.write(inputf+"\t"+"precision\n")
                    ...
                    if os.path.isfile(cesymm_rmsd_out_dir + inputf + ".axes"):
                        ...
                        if symmlist[inputf][name_dic]['repeats_number'] == 'na' or int(symmlist[inputf][name_dic]['repeats_number']) == 1:
                            ...
                            # or both has the same number of levels and greater coverage
                                    ...
                                    and symmlist[inputf][name_dic]['repeats_number'] != 'na' \
                                    and int(symmlist[inputf][name_dic]['repeats_number']) > 1:
                                # do not select the current symmetry if it has more levels with the same or worse coverage and same symmetry order
                                #if (int(symmlist[inputf][sel]['symmetry_levels']) < int(symmlist[inputf][name_dic]['symmetry_levels'])
                                #        and symmlist[inputf][sel]['symmetry_order'] == symmlist[inputf][name_dic]['symmetry_order']
                                #        and float(symmlist[inputf][sel]['coverage']) - float(symmlist[inputf][name_dic]['coverage'])>-0.1):
                                #    pass
                                if (int(symmlist[inputf][sel]['symmetry_levels']) < int(symmlist[inputf][name_dic]['symmetry_levels'])
                                    and (int(symmlist[inputf][name_dic]['repeat_length']) >= doubletmhelices
                                         and float(symmlist[inputf][sel]['coverage']) < float(symmlist[inputf][name_dic]['coverage']))) \
...
if __name__ == "__main__":
    ...
    #    options, filters, locations = filesystem_info("/data/TMB-CSB/Aleksandrova/EncoMPASS_1.1_2018_03_22/")
