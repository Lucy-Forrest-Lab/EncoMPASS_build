...
def ce_symm_minlen(cesymm_exe, out_dir, inputf, oriented, winsize, unrefinedtmscorethreshold, minlen, symmlevels, order,
    ...
    #   xmlfile="-xml=/Users/aleksandrovaaa/NIH_work/cesymm_output/"+inputf+"_output.xml"
    cmd = cesymm_exe.split() + ["-J"] + ["-stats=-"] + [rnd] + [oriented] + [xmlfile] + [fasta] + [axes] + [win] + [
...
def align_sequences(structA, structB, **kwargs):
    ...
    alns = PairwiseAligner.align.globalds(sequence_A, sequence_B,
...
def align_sequences_with_chains(wkdir, structA, structB, **kwargs):
    ...
    try:
        ...
        f.close()
        fnull = open(os.devnull, 'w')
        p = subprocess.Popen(muscle_path.split() + ['-align', wkdir + "tmp.fasta", '-output', wkdir + "out.fasta"],
...
def frtmalign_sequences_with_chains(wkdir, structA, structB, file, oriented_template, oriented_target, template, target,
    ...
    """
    frtmalign_path = kwargs.get('frtmalign_path', "/v/apps/csb/frtmalign/frtmalign.exe")
...
#    Note that chains is an array here, eg. ['A','B','C'].
    ...
    for chain in chains:
        f.seek(0)
        for line in f:
            if (line.startswith("ATOM") or line.startswith("TER ")) and line[21:22].strip()==chain and from3to1_general(line[17:20].strip())!=0:
                if line[76:78].strip()!='':
                    o.write(line)
                    num+=1
                else:
                    if line.startswith("TER "):
                        ...
                        o.write(LINELEM.format(line[0:76],elem))
                        num+=1
            if line.startswith("HETATM") and line[17:20]=='MSE' and line[21:22].strip()==chain:
                if line[76:78].strip()!='':
                    o.write("ATOM  "+line[6:])
                    num+=1
                else:
...
#	Note that chains can be either an array/list or a string.
    ...
    old_resid='-88'
    for line in f:
      if (line.startswith("ATOM") or line.startswith("TER ")) and (line[21:22] in chains or chains=="") and from3to1_general(line[17:20].strip())!=0:
        if old_resid!='-88' and line[22:27]==old_resid and line[12:16] in atomnames: #sort out disordered atoms
          continue
      ...
      if line.startswith("HETATM") and line[17:20]=='MSE' and (line[21:22] in chains or chains==""):
        if old_resid!='-88' and line[22:27]==old_resid and line[12:16] in atomnames:
          continue
        elif old_resid=='-88' or line[22:27]!=old_resid:
          old_resid=line[22:27]
          atomnames=[]
...
def symd_images(wkdir, symd_out_dir, inputf, oriented, ray, pymol, pic_dir, jmol_dir, aligned, aligned_chains):
    #    spectrumany_path="PATH/spectrumany.py"
    f = open(symd_out_dir + inputf + '-trfm.pdb', 'r')
    ...
    #    out.write("run "+spectrumany_path+"\n"
    #    out.write("spectrumany count, density lightblue, aligned\n")
    #    out.write("create not_aligned, "+inputf+" and not ("+sel_text+")\n")
    #    out.write("show cartoon, not_aligned\n")
    #    out.write("color grey70, not_aligned\n")
    #    out.write("set cartoon_transparency, 0.5, not_aligned\n")
    out.write("deselect\n")
    out.write("zoom " + inputf + "\n")
    out.write("cd " + pic_dir + "\n")
    ...
    #    out.write("load "+inputf+"-trfm.pdb, "+inputf+"_symd\n")
...
def get_repeats_alignment(wkdir,oriented,repeats):
   # 
   # Align multiple sequences to get the repeats alignment
   # 
    for i,rep in enumerate(repeats):
        num=strip_tm_chains_in_order(wkdir,'rep'+str(i),oriented,rep)

    res_maps=[]
    template=parse_structure(wkdir+'rep0_tmp.pdb')[0]
    for i in range(1,len(repeats)):
        target=parse_structure(wkdir+'rep'+str(i)+'_tmp.pdb')[0]
        aln, res_map = align_sequences_with_chains_and_gaps(wkdir,template, target)
        res_maps.append(res_map)

    # Figure out how many gaps the first repeat should have
    first_rep_aln=res_maps[0][0]  # example: [A, B, -, D, E, F], i.e. [(2, A),(3, A),(-,-),(5, A),(6, A),(7, A)]
    for i in range(1,len(res_maps)):
        if first_rep_aln==res_maps[i][0]:
            continue
        else:
            k=0
            for j,elem in enumerate(res_maps[i][0]): # example: [A, B, D, E, -, -, F]
                if first_rep_aln[k][0]=='-':
                    sub=first_rep_aln[k+1:]
                    ind=sub.index(elem)
                    k=k+ind+2
                elif elem[0]==first_rep_aln[k][0]:
                    k+=1
                elif elem[0]=='-':
                    first_rep_aln=first_rep_aln[0:k]+[elem]+first_rep_aln[k:]
                    k+=1
    
    # Get the other repeats to match the first repeat (form a multiple alignment)
    aln_map=[]
    for i,elem in enumerate(first_rep_aln):
        aln_map.append([elem])    
    for i in range(0,len(res_maps)):
        k=0
        for j,elem in enumerate(res_maps[i][0]):
            if elem[0]!='-':
                ind=first_rep_aln.index(elem)
                aln_map[ind].append(res_maps[i][1][j])
                k=ind+1
            else:
                aln_map[k].append(res_maps[i][1][j])
                k+=1
        for j,elem in enumerate(aln_map):
            if len(elem)<i+2:
                elem.append(('-','-'))

    for i in range(0,len(repeats)):        
        os.remove(wkdir+'rep'+str(i)+'_tmp.pdb')
    return aln_map
"""
...
def get_repeats_alignment(wkdir, oriented, repeats,
    ...
    fnull = open(os.devnull, 'w')
    p = subprocess.Popen(
        muscle_path.split() + ['-align', wkdir + "tmp-" + id + ".fasta", '-output', wkdir + "out-" + id + ".fasta"],
        stdout=fnull, stderr=fnull)
...
def cesymm_rmsd_tmscore(oriented, algn_map, repeats_type):
    ...
    #    io.save('/u/aa606/encompass_scripts/1yce_transform.pdb')
...
def transfer_symmetry(wkdir, frtm_path, cesymm_dir, transfer_out_dir, oriented_template, oriented_target, inputf,
    ...
    # commented out because to accommodate &amp; cases this needs to be done outside of this function
    """
    out=transfer_out_dir+inputf+"_transfer.out"
    ce_order=""
    if os.path.isfile(out):
        info=open(out,"r")
        for ln in info:
            if "template_order" in ln:
                fields=ln.split()
                ce_order=fields[1]  
    elif os.path.isfile(cesymm_dir+inputf+"_stdout.out") and ce_order=="":
        stdout=open(cesymm_dir+inputf+"_stdout.out","r")
        for ln in stdout:
            if inputf[0:4] in ln:
                fields=ln.split()
                ce_order=fields[2] #18
        ...
        raise SystemExit("Problem with the symmetry order determination.") 
    
    out=transfer_out_dir+target_pdb+"_transfer.out"
    output=open(out,"w")
    output.write("template_order\t"+ce_order+"\n")
    output.write("coverage\t"+str(float('%.2f' % (aligned/prot_len)))+"\n")
    output.write("size\t"+str(prot_len)+"\n")
    output.write("unit_angle\t"+angle+"\n")
    output.write("unit_translation\t"+translation+"\n")
    output.write("refined_tmscore\t"+str(float('%.2f' % tm_score))+"\n")
    output.write("refined_rmsd\t"+str(float('%.2f' % rmsd))+"\n")
    output.write("repeats_number\t"+str(num_repeats)+"\n")
    output.write("topology\t"+topology+"\n")
    output.write("axis_angle_with_membrane_normal\t"+axis_angle_with_membrane_normal+"\n")
    output.write("symmetry_levels\t"+str(symmetry_levels)+"\n")
    output.write("aligned_length\t"+str(aligned)+"\n")
    output.write("repeat_length\t"+str(rep_length)+"\n")
    output.write("template_structure\t"+inputf+"\n")
    output.close()
    parents=transfer_out_dir+target_pdb+"_transfer.parent"
    children=transfer_out_dir_template+inputf+"_transfer.children"
    ###
    if os.path.isfile(parents):
        with open(parents, "r") as f:
            prev_structures=f.readline().strip()
        output=open(parents,"w")
        output.write(prev_structures+","+inputf)
        output.close()
    else:
        output=open(parents,"w")
        output.write(inputf)
    ...
    if os.path.isfile(children):
        ...
        output.write(prev_structures+","+target_pdb)
        output.close()
    else:
        output=open(children,"w")
        output.write(target_pdb)
        output.close() 
    """
...
def create_tm_archive_new(locations, wkdir, arch_name):
    from encompass.pipeline.supporting_functions import read_checkpoint
    ...
    outfile = locations['SYSFILES']['tm_archive']
    pkl.dump(tm_archive, open(outfile, 'wb'))
    print("[INFO]: TM archive generated - " + outfile)
    return
