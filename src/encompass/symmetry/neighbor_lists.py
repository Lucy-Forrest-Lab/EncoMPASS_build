...
def choose_chain_representative_based_on_symm(struct_list, symm_rank_list, neighbors_list, tm_arch = False):
	...
	for cluster in neighbors_list:
		...
		for struct in cluster:
			unk = 0
			if tm_arch:
				if not struct[5:6] in tm_arch[struct[0:4]]:
					print(f"WARNING: a non-TM chain {struct} was compared to a TM chain. Eliminating it.")
					continue
...
if __name__ == "__main__":
	...
	tm_archive = locs['SYSFILES']['tm_archive']
