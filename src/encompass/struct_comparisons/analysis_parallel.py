...
from encompass.struct_comparisons import run_locusts_on_analysis
...
def equilibrium_ENM(posMX, relaxdMX, stiffMX, radialc):
    ...
    #    newposMX = scipy.optimize.minimize(energy_ENM, posMX.ravel(),
    #                  args=(relaxdMX, stiffMX),
    #                  method='L-BFGS-B').x.reshape((-1, 2))
    #    newposMX = newposMX - newposMX[0]  # re-center the self
    return newposMX
...
def colorbar_index(ncolors, cmap, ntm, cax=None, horiz=False):
    if horiz:
        ...
        #        colorbar.set_ticklabels(['#TM -5', '', '', '', '', 'Same #TM ({0})'.format(ntm), '', '', '', '', '#TM +5'])
        #        return
...
def neighborlists(options, sorted_table_fn, an_locations,
                  no_print=False):  # , only_dicts=False, only_databasewide=False):
    ...
    # TODO - check - these three values not used
    ntm_list = []
    ntmch_list = []
    ntmch_ntm = []
        ...
        with open(coord_fn, 'w') as coord_f:
            ...
            for i in range(ccoords.shape[0]):
                ...
                coord_f.write('{0}\t{1:8.5f}\t{2:8.5f}\t{3}\n'.format(tmlabels[i1][i], pcoords[i][0], pcoords[i][1],
                                                                      ntms[pdbi2][ch2]))
                # print(pdbi1, ch1, pdbi2, ch2, ntms[pdbi1][ch1], ntms[pdbi2][ch2], 0.5 - 0.1*(ntms[pdbi1][ch1] - ntms[pdbi2][ch2]))
...
def scatterplots_parallel(an_locations, entry_list, ntms, pkl_fn, do_eps=False, do_pdf=False):
    ...
    for i1, pdbi_ch1 in enumerate(entry_list):
        ...
        groups = set()
        for sname, px, py in clickmap:
            #            print("Adding",sname, px, py)
            added_to_group = False
            #            print("Groups:", groups)
            for g in groups:
                #            print("Consider group: ", g)
                not_in_this_group = False
                for elem in g:
                    d = ((elem[1] - px) ** 2 + (elem[2] - py) ** 2) ** 0.5
                    if (d > 5):
        ...
        print(" ".join(["/usr/bin/composite", layer_fn, an_locations['SYSFILES']['background'], figfilename]))
        msg = subprocess.run(["/usr/bin/composite", layer_fn, an_locations['SYSFILES']['background'], figfilename],
...
def scatterplots(options, locations, entry_list, ntms, an_locations, do_only=None, do_eps=False, do_pdf=False):
    ...
    DDx = np.array(DDx)
    DDy = np.array(DDy)
    # TODO - check - following four values not used
    xmin = DDx.min()
    xmax = DDx.max()
    ymin = DDy.min()
    ymax = DDy.max()
    ...
    for i1, pdbi_ch1 in enumerate(entry_list):
        ...
        ##cfset = ax.contourf(X, Y, ZZ, levels=levels, cmap=cmap, zorder=-2)
        ##cset = ax.contour(X, Y, ZZ, levels=levels, colors='k', zorder=-1)

        plt.locator_params(nbins=6)
        tt = ntms[pdbi1][ch1]  # table[table["PDB1"] == pdbi]["NTM1"].iloc[0].astype(int).values[0]
        # TODO - check - ndif not used?
        ndif = []
        XX = si[i1]
        YY = tm[i1]
        # XX = table[(table["PDB1"] == pdbi) & (table["CHAIN1"] == ch)]["SEQ_SEQID"].astype(float).values
        # YY = table[(table["PDB1"] == pdbi) & (table["CHAIN1"] == ch)]["TMSCORE"].astype(float).values
        ndif = []
        for i2, pdbi_ch2 in enumerate(labels[i1]):
            pdbi2, ch2 = pdbi_ch2.split('_')
            ndif.append(ntms[pdbi2][ch2] - tt)
        # ndif = table[(table["PDB1"] == pdbi) & (table["CHAIN1"] == ch)]["NTM2"].astype(int).values - tt
        ax.set_axisbelow(True)
        cmap.set_under("magenta")
        cmap.set_over("yellow")
        cmap.set_bad("red")
        ...
        for ng, g in enumerate(groups):
            ...
            if ng == len(groups) - 1:
                ...
                clickmapfile.write("{{\"X\":{0},\"Y\":{1:5d},\"radius\":3,\"chains\":\"{2}\"}},\n".format(cx, cy,
                                                                                                          ...
                                                                                                          ns[-1]))
        clickmapfile.write("]\n")
        #        print("{0:5d} {1:5d}\t\t{2}\n".format(cx, cy, "".join([x + " " for x in ns])))
...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    ...
    # TODO - check this is working - factorial is an unresolved reference
...
def straln_profiles(locations, entry_list, str_data, an_locations, do_eps=False, do_pdf=False):
    ...
    # colors = [np.array(cold[i]) for i in coll]
    print("COLORS", colors)
    # colors = ListedColormap(cols)
    print(entry_list)
    ...
    for pdbi_ch1 in entry_list:
        ...
        try:
            ...
            continue
        # print(xmax, xmin)
        # TODO - check here: expected type int, getting float
        ...
        if pdbi1 in str_data and ch1 in str_data[pdbi1]['ENCOMPASS']['structure']['ktmchains']:
            ...
            for ini, fin in str_data[pdbi1]['ENCOMPASS']['structure']['chains'][ich1]['TM_regions'][
                ...
                # print(pdbi1, ch1, str_data[pdbi1]['ENCOMPASS']['structure']['chains'][ich1]['TM_regions']['TM_regions_extrema'])
...
def draw_topology(options, locations, neighbors, names, opm_data, an_locations):
    ...
    def create_Bezier(BezierPoints, nsegments, radius, color='', gappy=False):
        ...
        for t in np.arange(0, 1 + 1 / nsegments, 1 / nsegments):
            ...
            for n in range(N + 1):
                # TODO - check this is working, ncr is an unresolved reference
...
def analysis_archive(locations, analysis_path, generate=True, analysis_name='analysis/'):
    ...
    an_locations['SYSFILES']['strdata'] = an_locations['FSYSPATH']['cache'] + 'str_data_completegen.pkl'
...
def graphical_analysis(options, locations, str_data, analysisdir='None'):
    ...
    scatterplots(options, locations, pdbi_chs, ntms, an_locations, do_eps=False)
    # return
    print("POLAR")
    #    dos = ['1bl8', '1f88', '1okc', '4c7r', '4qnc']
    #    do_only = []
    #    for s in pdbi_chs:
    #        if s[:4] in dos:
    #            do_only.append(s)
    ...
    straln_profiles(locations, do_only, str_data, an_locations, do_eps=False)  # , do_only=rlist, do_eps=True)
    return
...
def parallel_graphical_analysis(options, locations, str_data, do_only, ntms, sp_pkl_fn, analysisdir='None'):
    ...
    straln_profiles(an_locations, do_only, str_data, an_locations, do_eps=False)
    ...
    for ic1 in range_list:
        for ic2 in range_list:
            all_entries += [os.path.basename(x[0][-10:-4]) for x in [y.decode('utf8').split() for y in subprocess.Popen(
                ...
                shell=True, stdout=subprocess.PIPE).stdout] if x]
    do_only = all_entries
    straln_profiles(locations, do_only, str_data, an_locations, do_eps=False)
...
### OUTDATED: CONVERT THIS INTO PANDAS?
def extract_ntms(fn):
    xinf = {}
    first = True
    # print(fn)
...
def missing_entries_gen():
    ...
    for ic1 in ords:
        for ic2 in ords:
            #print("ls {0}/strneigh_?{1}{2}*".format(locations['FSYSPATH']['strneighs'], chr(ic1), chr(ic2)))
            all_entries += [os.path.basename(x[0][-10:-4]) for x in [y.decode('utf8').split() for y in subprocess.Popen(
...
if __name__ == "__main__":
    ...
    if sys.argv[-1] in kw:  # PARALLEL MUST BE PUT AS LAST ARGUMENT
        ...
        if options['ALL']['filter_entries_analysis']:
            ...
            ### IF YOU WANT TO FILL THE MISSING ONES
            proto_do_onlys = set()
        ...
        command = '{0} {1} <arg0> <arg1> <arg2> <arg3> <arg4> <arg5> '.format(
                options['ALL']['hpcactipy'].replace("activate","python3"),
        ...
        ##subprocess.run(["tar", "-czvf", an_locations['FSYSPATH']['distributions_data'].rstrip("/")+".tgz", an_locations['FSYSPATH']['distributions_data']])
        ...
        shutil.copyfile(locations['FSYSPATH']['cache'] + 'str_data_completegen.pkl', str_pkl)
